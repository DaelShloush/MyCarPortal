...
| **לוחות רכב (יד2 וכו')** | מודעות מכירה | ממוקד במוכר; אין אימות נתונים רשמי, אין ניהול רכב קיים ותזכורות |
| **שירותי "בדיקת עבר רכב" בתשלום** | דוח חד-פעמי | בתשלום, חד-פעמי, מסתיים ברגע הקנייה — לא מלווה אותך לאורך הבעלות |
| **אפליקציות ניהול רכב (Drivvo וכד')** | תזכורות ויומן טיפולים | לא מחוברות לנתוני data.gov.il הישראליים; ידני לגמרי |
| **"לעשות ידנית" — אקסל / תזכורות בטלפון / תיקיית מסמכים** | גמיש, מוכר | שביר, נשכח, בלי נתונים רשמיים ובלי הערכת שווי |
...
- **מלווה לאורך כל הבעלות, לא רק בקנייה** — תזכורות טסט/ביטוח במייל, אחסון מסמכים והיסטוריית טיפולים — מה שלוחות הרכב ושירותי הבדיקה לא נותנים.
...
> **אבטחת מפתחות:** כל המפתחות והסודות (Supabase service role, Resend, Google secret) נשמרים כמשתני סביבה בצד השרת בלבד ולעולם לא נחשפים לקליינט. קריאות ל-data.gov.il מתבצעות דרך proxy בשרת.
